...
cli() { env -i HOME="$root/home" PATH="$PATH" openshell "$@"; }
case "${1:-}" in
start)
...
create) cli -g workbench-t0 sandbox create --name workbench-t0-probe --no-auto-providers --detach --no-tty -- sh -c 'sleep 1200' ;;
...
*) echo 'usage: native-probe.sh start|register|status|create|logs|stop|cli ...' >&2; exit 2 ;;
esac
